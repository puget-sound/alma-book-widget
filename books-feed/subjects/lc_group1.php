<?php

//LC Group 1 Classifications - Subject Linking
//Add terms to Switch Statement 

foreach($group1 as $term) {

	switch ($term) {
				case "Aesthetics":       
						array_push($subjects, "Philosophy");       
						break;
				case "Afrikaans literature":       
						array_push($subjects, "African American Studies");       
						break;
				case "Agriculture (General)":       
						array_push($subjects, "Biology");       
						break;
				case "Agriculture (General)":       
						array_push($subjects, "Environmental Policy & Decision Making");       
						break;
				case "Agriculture (General)":       
						array_push($subjects, "International Political Economy");       
						break;
				case "America":       
						array_push($subjects, "History");       
						break;
				case "American literature":       
						array_push($subjects, "English");       
						break;
				case "Animal culture":       
						array_push($subjects, "Biology");       
						break;
				case "Animal culture":       
						array_push($subjects, "Environmental Policy & Decision Making");       
						break;
				case "Animal culture":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Anthropology":       
						array_push($subjects, "Sociology & Anthropology");       
						break;
				case "Aquaculture. Fisheries. Angling":       
						array_push($subjects, "Biology");       break;
				case "Aquaculture. Fisheries. Angling":       
						array_push($subjects, "Environmental Policy & Decision Making");       
						break;
				case "Architecture":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Armies: Organization, distribution, military situation":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Arts in general":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Asia and Eurasia, Africa, Pacific Area, and Antarctica":       
						array_push($subjects, "Asian Studies");       
						break;
				case "Astronomy":       
						array_push($subjects, "Physics");       
						break;
				case "Biography":       
						array_push($subjects, "History");       
						break;
				case "Biology (General)":       
						array_push($subjects, "Biology");       
						break;
				case "Books (General). Writing. Paleography":       
						array_push($subjects, "History");       
						break;
				case "Botanic, Thomsonian, and eclectic medicine":       
						array_push($subjects, "Science, Technology & Society");       
						break;
				case "Botany":       
						array_push($subjects, "Biology");       
						break;
				case "British America":       
						array_push($subjects, "History");       
						break;
				case "Buddhism":       
						array_push($subjects, "Religious Studies");       
						break;
				case "Buddhism":       
						array_push($subjects, "Asian Studies");       
						break;
				case "Buddhism":       
						array_push($subjects, "Philosophy");       
						break;
				case "Byzantine and modern Greek literature":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "Canadian literature":       
						array_push($subjects, "English");       
						break;
				case "Chemical technology":       
						array_push($subjects, "Chemistry");       
						break;
				case "Chemical technology":       
						array_push($subjects, "Biochemistry & Molecular Biology");       
						break;
				case "Chemistry":       
						array_push($subjects, "Chemistry");       
						break;
				case "Christian Denominations":       
						array_push($subjects, "Religious Studies");       
						break;
				case "Christianity":       
						array_push($subjects, "Religious Studies");       
						break;
				case "Classical literature":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "Colonies and colonization. Emigration and immigration. International migration":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Commerce":       
						array_push($subjects, "Business & Leadership");       
						break;
				case "Commerce":       
						array_push($subjects, "Economics");       
						break;
				case "Commerce":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Communities. Classes. Races":       
						array_push($subjects, "African American Studies");       
						break;
				case "Communities. Classes. Races":       
						array_push($subjects, "Sociology & Anthropology");       
						break;
				case "Danish literature":       
						array_push($subjects, "History");       
						break;
				case "Decorative arts":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Dentistry":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Dermatology":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Doctrinal Theology":       
						array_push($subjects, "Religious Studies");       
						break;
				case "Drawing. Design. Illustration":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Dutch literature":       
						array_push($subjects, "History");       
						break;
				case "Economic history and conditions":       
						array_push($subjects, "Economics");       
						break;
				case "Economic history and conditions":       
						array_push($subjects, "Business & Leadership");       
						break;
				case "Economic history and conditions":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Economic history and conditions":       
						array_push($subjects, "History");       
						break;
				case "Economic theory. Demography":       
						array_push($subjects, "Economics");       
						break;
				case "Economic theory. Demography":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Electrical engineering. Electronics. Nuclear engineering":       
						array_push($subjects, "Engineering");       
						break;
				case "Engineering (General). Civil engineering (General)":       
						array_push($subjects, "Engineering");       
						break;
				case "English":       
						array_push($subjects, "English");       
						break;
				case "English literature":       
						array_push($subjects, "English");       
						break;
				case "Environmental sciences":       
						array_push($subjects, "Environmental Policy & Decision Making");       
						break;
				case "Environmental sciences":       
						array_push($subjects, "Biology");       
						break;
				case "Environmental technology. Sanitary engineering":       
						array_push($subjects, "Environmental Policy & Decision Making");       
						break;
				case "Ethics":       
						array_push($subjects, "Philosophy");       
						break;
				case "Ethics":       
						array_push($subjects, "Bioethics");       
						break;
				case "Europe":       
						array_push($subjects, "History");       
						break;
				case "Faroese literature":       
						array_push($subjects, "History");       
						break;
				case "Fiction and juvenile belles lettres":       
						array_push($subjects, "English");       
						break;
				case "Fiction and juvenile belles lettres":       
						array_push($subjects, "Education Studies");       
						break;
				case "Finance":       
						array_push($subjects, "Business & Leadership");       
						break;
				case "Finance":       
						array_push($subjects, "Economics");       
						break;
				case "Finance":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Flemish literature since 1830":       
						array_push($subjects, "History");       
						break;
				case "Forestry":       
						array_push($subjects, "Biology");       
						break;
				case "Forestry":       
						array_push($subjects, "Environmental Policy & Decision Making");       
						break;
				case "French literature":       
						array_push($subjects, "French Studies");       
						break;
				case "Geography (General)":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Geology":       
						array_push($subjects, "Geology");       
						break;
				case "German literature":       
						array_push($subjects, "German Studies");       
						break;
				case "Greek literature":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "Greek philology and language":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "Gynecology and obstetrics":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Handicrafts. Arts and crafts":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Heraldry":       
						array_push($subjects, "History");       
						break;
				case "History (General)":       
						array_push($subjects, "History");       
						break;
				case "History of Africa":       
						array_push($subjects, "History");       
						break;
				case "History of Africa":       
						array_push($subjects, "African American Studies");       
						break;
				case "History of Asia":       
						array_push($subjects, "Asian Studies");       
						break;
				case "History of Asia":       
						array_push($subjects, "History");       
						break;
				case "History of Austria. Liechtenstein. Hungary. Czechoslovakia":       
						array_push($subjects, "History");       
						break;
				case "History of Balkan Peninsula":       
						array_push($subjects, "History");       
						break;
				case "History of canon law":       
						array_push($subjects, "History");       
						break;
				case "History of Central Europe":       
						array_push($subjects, "German Studies");       
						break;
				case "History of Central Europe":       
						array_push($subjects, "History");       
						break;
				case "History of Civilization":       
						array_push($subjects, "History");       
						break;
				case "History of Civilization":       
						array_push($subjects, "Sociology & Anthropology");       
						break;
				case "History of Eastern Europe (General)":       
						array_push($subjects, "History");       
						break;
				case "History of education":       
						array_push($subjects, "Education Studies");       
						break;
				case "History of education":       
						array_push($subjects, "History");       
						break;
				case "History of France":       
						array_push($subjects, "French Studies");       
						break;
				case "History of France":       
						array_push($subjects, "History");       
						break;
				case "History of Germany":       
						array_push($subjects, "German Studies");       
						break;
				case "History of Germany":       
						array_push($subjects, "History");       
						break;
				case "History of Great Britain":       
						array_push($subjects, "History");       
						break;
				case "History of Greece":       
						array_push($subjects, "History");       
						break;
				case "History of Greece":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "History of Italy":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "History of Italy":       
						array_push($subjects, "History");       
						break;
				case "History of Low Countries. Benelux Countries":       
						array_push($subjects, "History");       
						break;
				case "History of Netherlands (Holland)":       
						array_push($subjects, "History");       
						break;
				case "History of Northern Europe. Scandinavia":       
						array_push($subjects, "History");       
						break;
				case "History of Oceania (South Seas)":       
						array_push($subjects, "Asian Studies");       
						break;
				case "History of Oceania (South Seas)":       
						array_push($subjects, "History");       
						break;
				case "History of Poland":       
						array_push($subjects, "History");       
						break;
				case "History of Portugal":       
						array_push($subjects, "Hispanic Studies");       
						break;
				case "History of Portugal":       
						array_push($subjects, "History");       
						break;
				case "History of Romanies":       
						array_push($subjects, "History");       
						break;
				case "History of Russia. Soviet Union. Former Soviet Republics":       
						array_push($subjects, "History");       
						break;
				case "History of scholarship and learning. The humanities":       
						array_push($subjects, "History");       
						break;
				case "History of Spain":       
						array_push($subjects, "History");       
						break;
				case "History of Spain":       
						array_push($subjects, "Hispanic Studies");       
						break;
				case "History of Switzerland":       
						array_push($subjects, "History");       
						break;
				case "History of the Greco-Roman world":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "History of the Greco-Roman world":       
						array_push($subjects, "History");       
						break;
				case "Homeopathy":       
						array_push($subjects, "Science, Technology & Society");       
						break;
				case "Human anatomy":       
						array_push($subjects, "Exercise Science");       
						break;
				case "Human anatomy":       
						array_push($subjects, "Biology");       
						break;
				case "Human anatomy":       
						array_push($subjects, "Physical Therapy");       
						break;
				case "Human anatomy":       
						array_push($subjects, "Neuroscience");       
						break;
				case "Human anatomy":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Human ecology. Anthropogeography":       
						array_push($subjects, "Sociology & Anthropology");       
						break;
				case "Industries. Land use. Labor":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Industries. Land use. Labor":       
						array_push($subjects, "Business & Leadership");       
						break;
				case "Internal medicine":       
						array_push($subjects, "Science, Technology & Society");       
						break;
				case "Internal medicine":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Internal medicine":       
						array_push($subjects, "Physical Therapy");       
						break;
				case "International law, see JZ and KZ":       
						array_push($subjects, "International Political Economy");       
						break;
				case "International relations":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Islam. Bahai Faith. Theosophy, etc.":       
						array_push($subjects, "Asian Studies");       
						break;
				case "Islam. Bahai Faith. Theosophy, etc.":       
						array_push($subjects, "Religious Studies");       
						break;
				case "Judaism":       
						array_push($subjects, "Religious Studies");       
						break;
				case "Languages of Eastern Asia, Africa, Oceania":       
						array_push($subjects, "Asian Studies");       
						break;
				case "Languages of Eastern Asia, Africa, Oceania":       
						array_push($subjects, "Asian Languages & Cultures");       
						break;
				case "Latin America (General)":       
						array_push($subjects, "Latin American Studies");       
						break;
				case "Latin America. Spanish America":       
						array_push($subjects, "Latina/o Studies");       
						break;
				case "Latin America. Spanish America":       
						array_push($subjects, "Latin American Studies");       
						break;
				case "Latin America. Spanish America":       
						array_push($subjects, "Hispanic Studies");       
						break;
				case "Latin philology and language":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "Law in general. Comparative and uniform law. Jurisprudence":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Law of nations":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Law of the United Kingdom and Ireland":       
						array_push($subjects, "History");       
						break;
				case "Law of the United States":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Literature (General)":       
						array_push($subjects, "English");       
						break;
				case "Literature on music":       
						array_push($subjects, "Music");       
						break;
				case "Local government. Municipal government":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Manners and customs (General)":       
						array_push($subjects, "History");       
						break;
				case "Manufactures":       
						array_push($subjects, "Engineering");       
						break;
				case "Mathematical geography. Cartography":       
						array_push($subjects, "Mathematics");       
						break;
				case "Mathematics":       
						array_push($subjects, "Mathematics");       
						break;
				case "Mechanical engineering and machinery":       
						array_push($subjects, "Engineering");       
						break;
				case "Medicine (General)":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Medicine (General)":       
						array_push($subjects, "Bioethics");       
						break;
				case "Medieval and modern Greek language":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "Medieval and modern Latin literature":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "Military administration":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Military engineering":       
						array_push($subjects, "Engineering");       
						break;
				case "Military science (General)":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Modern Icelandic literature":       
						array_push($subjects, "History");       
						break;
				case "Motor vehicles. Aeronautics. Astronautics":       
						array_push($subjects, "Engineering");       
						break;
				case "Music":       
						array_push($subjects, "Music");       
						break;
				case "Natural history (General)":       
						array_push($subjects, "Biology");       
						break;
				case "Natural history (General)":       
						array_push($subjects, "Science, Technology & Society");       
						break;
				case "Natural history (General)":       
						array_push($subjects, "Geology");       
						break;
				case "Norwegian literature":       
						array_push($subjects, "History");       
						break;
				case "Nursing":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Oceanography":       
						array_push($subjects, "Environmental Policy & Decision Making");       
						break;
				case "Old Icelandic and Old Norwegian":       
						array_push($subjects, "History");       
						break;
				case "Old Norse literature":       
						array_push($subjects, "History");       
						break;
				case "Ophthalmology":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Oriental philology and literature":       
						array_push($subjects, "Asian Languages & Cultures");       
						break;
				case "Oriental philology and literature":       
						array_push($subjects, "Asian Studies");       
						break;
				case "Otorhinolaryngology":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Painting":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Pathology":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Pediatrics":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Pharmacy and materia medica":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Philology. Linguistics":       
						array_push($subjects, "Communication Studies");       
						break;
				case "Philosophy (General)":       
						array_push($subjects, "Philosophy");       
						break;
				case "Photography":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Physical geography":       
						array_push($subjects, "Geology");       
						break;
				case "Physics":       
						array_push($subjects, "Physics");       
						break;
				case "Physiology":       
						array_push($subjects, "Exercise Science");       
						break;
				case "Physiology":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Physiology":       
						array_push($subjects, "Physical Therapy");       
						break;
				case "Plant culture":       
						array_push($subjects, "Biology");       
						break;
				case "Political institutions and public administration":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Political institutions and public administration (Asia,":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Political institutions and public administration (Asia, Africa, Australia, Pacific Area, etc.)":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Political institutions and public administration (Europe)":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Political institutions and public administration (Europe)":       
						array_push($subjects, "International Political Economy");       
						break;
				case "Political institutions and public administration (United States)":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Political science (General)":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Political theory. The state. Theories of the state":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Portuguese literature":       
						array_push($subjects, "Latina/o Studies");       
						break;
				case "Portuguese literature":       
						array_push($subjects, "Hispanic Studies");       
						break;
				case "Portuguese literature":       
						array_push($subjects, "Latin American Studies");       
						break;
				case "Practical Theology":       
						array_push($subjects, "Religious Studies");       
						break;
				case "Print media":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Psychology":       
						array_push($subjects, "Psychology");       
						break;
				case "Psychology":       
						array_push($subjects, "Neuroscience");       
						break;
				case "Public aspects of medicine":       
						array_push($subjects, "Bioethics");       
						break;
				case "Public finance":       
						array_push($subjects, "Business & Leadership");       
						break;
				case "Public finance":       
						array_push($subjects, "Economics");       
						break;
				case "Recreation. Leisure":       
						array_push($subjects, "Physical Education");       
						break;
				case "Religions. Mythology. Rationalism":       
						array_push($subjects, "Religious Studies");       
						break;
				case "Roman literature":       
						array_push($subjects, "Classics & Ancient Mediterranean Studies");       
						break;
				case "Scandinavian literature":       
						array_push($subjects, "History");       
						break;
				case "Science (General)":       
						array_push($subjects, "Biochemistry & Molecular Biology");       
						break;
				case "Science (General)":       
						array_push($subjects, "Biology");       
						break;
				case "Science (General)":       
						array_push($subjects, "Bioethics");       
						break;
				case "Science (General)":       
						array_push($subjects, "Chemistry");       
						break;
				case "Science (General)":       
						array_push($subjects, "Physics");       
						break;
				case "Science (General)":       
						array_push($subjects, "Science, Technology & Society");       
						break;
				case "Sculpture":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "Social history and conditions. Social problems. Social reform":       
						array_push($subjects, "History");       
						break;
				case "Social history and conditions. Social problems. Social reform":       
						array_push($subjects, "Sociology & Anthropology");       
						break;
				case "Social pathology. Social and public welfare. Criminology":       
						array_push($subjects, "Sociology & Anthropology");       
						break;
				case "Socialism. Communism. Anarchism":       
						array_push($subjects, "Politics & Government");       
						break;
				case "Sociology":       
						array_push($subjects, "Sociology & Anthropology");       
						break;
				case "Spanish literature":       
						array_push($subjects, "Latina/o Studies");       
						break;
				case "Spanish literature":       
						array_push($subjects, "Hispanic Studies");       
						break;
				case "Spanish literature":       
						array_push($subjects, "Latin American Studies");       
						break;
				case "Special aspects of education":       
						array_push($subjects, "Education Studies");       
						break;
				case "Speculative philosophy":       
						array_push($subjects, "Philosophy");       
						break;
				case "Statistics":       
						array_push($subjects, "Mathematics");       
						break;
				case "Swedish literature":       
						array_push($subjects, "History");       
						break;
				case "Technology (General)":       
						array_push($subjects, "Science, Technology & Society");       
						break;
				case "Technology (General)":       
						array_push($subjects, "Engineering");       
						break;
				case "The Bible":       
						array_push($subjects, "Religious Studies");       
						break;
				case "The Family. Marriage. Women":       
						array_push($subjects, "Gender & Queer Studies");       
						break;
				case "Theory and practice of education":       
						array_push($subjects, "Education Studies");       
						break;
				case "Therapeutics. Pharmacology":       
						array_push($subjects, "Occupational Therapy");       
						break;
				case "Therapeutics. Pharmacology":       
						array_push($subjects, "Neuroscience");       
						break;
				case "United States":       
						array_push($subjects, "History");       
						break;
				case "United States local history":       
						array_push($subjects, "History");       
						break;
				case "Uralic. Basque":       
						array_push($subjects, "Hispanic Studies");       
						break;
				case "Visual arts":       
						array_push($subjects, "Arts & Art History");       
						break;
				case "West Germanic":       
						array_push($subjects, "German Studies");       
						break;
				case "Zoology":       
						array_push($subjects, "Biology");       
						break;
	}//End Switch
}//End foreach


?>
